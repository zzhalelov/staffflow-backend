...
@WebMvcTest({OrganizationController.class, OrganizationMapper.class})
...
class OrganizationControllerTest {
    ...
    void findAll_shouldReturnNonEmptyList() {
        ...
        Page<Organization> page =
                new PageImpl<>(
                        List.of(organization1, organization2),
                        PageRequest.of(0, 20),
                        2
                );

        Mockito
                .when(organizationService.findAll(Mockito.any(Pageable.class)))
                .thenReturn(page);

        mockMvc.perform(get("/api/organizations"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content.length()").value(2))
                .andExpect(jsonPath("$.content[0].id").value(organization1.getId()))
